...
const parseTopicProgress = (raw: string): TopicProgress[] => {
  try {
    ...
    return parsed.flatMap((item) => {
      ...
      if (
        ...
        !isTopicStatus(candidate.status)
      )
        return [];
...
export const readTopicProgress = (): TopicProgress[] => {
...
export const writeTopicProgress = (progress: TopicProgress[]): void => {
  if (globalThis.window === undefined) return;

  try {
    globalThis.localStorage.setItem(
      TOPIC_PROGRESS_STORAGE_KEY,
...
export const getTopicStatus = (topicId: string): TopicStatus => {
  const progress = readTopicProgress();
  const entry = progress.find((item) => item.topicId === topicId);
  return entry?.status ?? "new";
};

export const setTopicStatus = (topicId: string, status: TopicStatus): void => {
  const progress = readTopicProgress();
